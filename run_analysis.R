...
y_train = read.csv("UCI HAR Dataset/train/y_train.txt", sep="", header=FALSE, col.names=c("Activity_Label"))
subject_train= read.csv("UCI HAR Dataset/train/subject_train.txt", sep="", header=FALSE, col.names=c("subject"))

##Load test data
X_test = read.csv("UCI HAR Dataset/test/X_test.txt", sep="", header=FALSE)
y_test = read.csv("UCI HAR Dataset/test/y_test.txt", sep="", header=FALSE, col.names=c("Activity_Label"))
...
## 2. Extracts only the measurements on the mean and standard deviation for each measurement.
all_labels <- rbind(rbind(features, c(562,"subject")),c(563, "Activity_Label"))[,2]
names(all_data) <- all_labels

all_data<-all_data[,grepl("mean|std|subject|Activity_Labeld", names(all_data))]

## 3. Uses descriptive activity names to name the activities in the data set
all_data <- join(all_data, activity_labels, by = "Activity_Label", match = "first")
all_data <- all_data[,-1]
...
## 5. Creates a second, independent tidy data set with the average of each variable for each activity and each subject.
tidy_data<-ddply(all_data, c("subject","Activity_Label"), numcolwise(mean))
